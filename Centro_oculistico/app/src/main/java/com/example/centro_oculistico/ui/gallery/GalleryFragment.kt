package com.example.centro_oculistico.ui.gallery
...
import com.example.centro_oculistico.databinding.FragmentGalleryBinding
import com.example.centro_oculistico.ui.slideshow.SlideshowFragmentDirections
...
private lateinit var editor : SharedPreferences.Editor
    ...
    override fun onCreateView(
        ...
        var doctor: MutableList<Doctor> = mutableListOf()
        //http call to get the resources
        val queue = Volley.newRequestQueue(activity)
        val url = "http://centrooculistico.hostinggratis.it/centro_oculisticoREST/doctor/read.php"
        ...
        val jsonArrayRequest = JsonArrayRequest(Request.Method.GET, url, null,
            { response ->
                for (i in 0 until response.length()) {
                    ...
                    val itemDoctor = Doctor(
                        item.get("doctorId").toString(),
                        item.get("name").toString(),
                        item.get("surname").toString(),
                        item.get("dateOfBirth").toString(),
                        item.get("email").toString(),
                        item.get("phoneNumber").toString(),
                        item.get("headOffice").toString(),
                        item.get("passwd").toString(),
                        item.get("gender").toString()
                    )
                    doctor.add(itemDoctor)
                    // Your code here
                }


                //textView.text = "Response: %s".format(jsonObject.get("doctorId").toString())

                recyclerView = binding.recyclerViewDoctor
                recyclerView.adapter = DoctorItemAdapter(context,doctor,parentFragmentManager)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_logout -> {
                ...
                val action= GalleryFragmentDirections.actionNavGalleryToNavHome()
